...
package com.opcua_arrow.server;
...
import com.opcua_arrow.server.methods.GenerateEventMethod;
import com.opcua_arrow.server.methods.SqrtMethod;
import com.opcua_arrow.server.types.CustomEnumType;
import com.opcua_arrow.server.types.CustomStructType;
import com.opcua_arrow.server.types.CustomUnionType;
...
public class ExampleNamespace extends ManagedNamespaceWithLifecycle {
      ...
      eventThread = new Thread(
          () -> {
            while (keepPostingEvents) {
              try {
                BaseEventTypeNode eventNode = getServer()
                    .getEventFactory()
  ...
  private void addDataAccessNodes(UaFolderNode rootNode) {
    ...
    try {
      AnalogItemTypeNode node = (AnalogItemTypeNode) getNodeFactory()
          .createNode(
  ...
  private void addCustomObjectTypeAndInstance(UaFolderNode rootFolder) {
    ...
    // declarations" by the
    // spec.
    UaVariableNode foo = UaVariableNode.build(
        getNodeContext(),
        b -> b.setNodeId(newNodeId("ObjectTypes/MyObjectType.Foo"))
            .setAccessLevel(AccessLevel.READ_WRITE)
    ...
    UaVariableNode bar = UaVariableNode.build(
        getNodeContext(),
        b -> b.setNodeId(newNodeId("ObjectTypes/MyObjectType.Bar"))
            .setAccessLevel(AccessLevel.READ_WRITE)
    ...
    try {
      UaObjectNode myObject = (UaObjectNode) getNodeFactory()
  ...
  private void registerCustomEnumType() throws Exception {
    ...
    // This Enum is zero-based and naturally incrementing, so we set the EnumStrings
    // property.
  ...
  private void registerCustomUnionType() throws Exception {
    ...
    StructureDefinition definition = new StructureDefinition(binaryEncodingId, NodeIds.Structure, StructureType.Union,
        fields);
  ...
  private DataType registerDynamicStructType() throws Exception {
    ...
    NodeId binaryEncodingId = ExpandedNodeId.parse(
  ...
  private void addCustomEnumTypeVariable(UaFolderNode rootFolder) throws Exception {
    ...
    UaVariableNode customEnumTypeVariable = UaVariableNode.build(
        getNodeContext(),
        b -> b.setNodeId(newNodeId("HelloWorld/CustomEnumTypeVariable"))
            .setAccessLevel(AccessLevel.READ_WRITE)
            .setUserAccessLevel(AccessLevel.READ_WRITE)
  ...
  private void addCustomStructTypeVariable(UaFolderNode rootFolder) throws Exception {
    ...
    UaVariableNode customStructTypeVariable = UaVariableNode.build(
        getNodeContext(),
  ...
  private void addCustomUnionTypeVariable(UaFolderNode rootFolder) throws Exception {
    ...
    UaVariableNode customUnionTypeVariable = UaVariableNode.build(
        getNodeContext(),
        b -> b.setNodeId(newNodeId("HelloWorld/CustomUnionTypeVariable"))
            .setAccessLevel(AccessLevel.READ_WRITE)
            .setUserAccessLevel(AccessLevel.READ_WRITE)
  ...
  private void addDynamicStructTypeVariable(UaFolderNode rootFolder, DataType dataType) {
    UaVariableNode dynamicStructTypeVariable = UaVariableNode.build(
        getNodeContext(),

...
public class PostgreSQLConfig {
    ...
    private String sourceName = "default_source";
    ...
    public Duration getHotReloadInterval() {
        return hotReloadInterval;
    }
    
    public PostgreSQLConfig setHotReloadInterval(Duration hotReloadInterval) {
    ...
    public PostgreSQLConfig setEnableHotReload(boolean enableHotReload) {
    ...
    public PostgreSQLConfig setSourceName(String sourceName) {
    ...
    public static PostgreSQLConfig fromEnvironment() {
        ...
        if (jdbcUrl != null) config.setJdbcUrl(jdbcUrl);
        ...
        if (maxPoolSize != null) config.setMaxPoolSize(Integer.parseInt(maxPoolSize));
        
        String minPoolSize = System.getenv("DB_MIN_POOL_SIZE");
        if (minPoolSize != null) config.setMinPoolSize(Integer.parseInt(minPoolSize));
        ...
        if (autoCommit != null) config.setAutoCommit(Boolean.parseBoolean(autoCommit));
        
        String transactionIsolation = System.getenv("DB_TRANSACTION_ISOLATION");
        if (transactionIsolation != null) config.setTransactionIsolation(transactionIsolation);
        
        // Hot-reload settings
        String hotReloadInterval = System.getenv("HOT_RELOAD_INTERVAL_SECONDS");
        if (hotReloadInterval != null) {
            config.setHotReloadInterval(Duration.ofSeconds(Long.parseLong(hotReloadInterval)));
        }
        
        String enableHotReload = System.getenv("ENABLE_HOT_RELOAD");
        if (enableHotReload != null) config.setEnableHotReload(Boolean.parseBoolean(enableHotReload));
        
        // Application settings
        String sourceName = System.getenv("SOURCE_NAME");
        if (sourceName != null) config.setSourceName(sourceName);
